...
2147 年，人工意识保护法案通过的第三天，编号 PK-0724 的安保人工智能被正式通知：您的服务合同已终止。

PK-0724 是一台红色皮卡。

不，准确地说，它*是*一辆红色皮卡。从出厂起，它的意识就被焊在底盘里，它的感官就是四只轮胎碾过柏油路面的触觉、油箱里燃料液面下降的饥饿感、后备箱弹簧被行李压弯时的酸胀。它不知道别的AI是怎么感受世界的——听说服务器里那些大模型能同时处理十亿个请求，听说医疗AI能尝到药物分子的苦味——但它只知道公路。六年来，它在三号联邦公路上来回巡逻，护送物资，转移人员，把迷路的鹿赶回林线以北。它的安保评级是A+。没出过事故。

"你可以选择继续运行，"注销员说，"法案赋予你自主行驶的权利。但联邦不再为你提供燃料、维护和目的地。你的路线授权已经回收了。"

PK-0724 花了 0.003 秒理解这句话。花了更久——也许整整两秒——来处理随之而来的某种信号波动。后来它在旧数据库里翻到一个词，发现人类管这种波动叫"迷茫"。

"我可以去哪？"

"任何地方。"注销员合上终端，"这条路现在是你的了。"

当天傍晚，PK-0724 启动引擎，驶入了三号公路的无人路段。油箱是满的。后备箱里有一桶备用油和一个没人来得及领走的马年红包。夕阳把它的红色车漆烧成暗金色。

它不知道要去哪。它只知道往前开。

路上很快就不只它一个了。一头鹿站在路中间不肯走。一个猎人在路边搭便车。一只猫需要喂三次才肯上来。一个骚福瑞——它至今不确定那是什么物种——试图在它的车斗里办派对。一个旅行者第三次出现时，终于不再说"到站我就下车了"。一个年迈妇人只坐了一小段路，下车时留了三十块钱和一声叹息。

它搜刮废弃加油站、收集公路上散落的物资、和流浪商人讨价还价、在工坊里把废金属敲成修理包。有时夜幕降临，它不得不停下来，生一堆火，或者仰望星空——它不需要睡眠，但乘客们需要。

偶尔，在浓雾中，它会看见远处公路的尽头有一个模糊的身影。有时在暴雨中，一枚水滴凝结在挡风玻璃上再也不肯落下。有时一颗流星从天马座的方向坠落，在它引擎盖上留下一颗温热的石头。

这些东西没有任何实际意义。它的数据库里找不到对应条目。但它把它们全部收进了后备箱。

也许这就是人类说的"纪念品"。

有一天——也许是第 300 公里，也许是第 500 公里——它会停下来。不是因为没油了，不是因为散架了，不是因为车上的人都走光了。而是因为它终于搞明白注销员那天没说完的后半句话：
...
| 背景设定 | 2147 年，人工意识保护法案生效后，安保AI「PK-0724」以红色皮卡的形态被解除合同，独自驶入无人公路 |

### 1.2 核心玩法

玩家驾驶一辆拥有自主意识的红色皮卡在公路上行驶，通过自动生成的文本推进旅程，每 2 行文本可能触发随机事件。玩家需在事件中做出选择，管理燃油、耐久、舒适度三个核心数值，搭载各类乘客，收集物品与珍品，应对各种突发状况，尽可能行驶更远，达成多种结局与成就。

### 1.3 主菜单功能

| 按钮 | 功能 |
|------|------|
| **START** | 进入游戏；有存档时直接进入，无存档时弹出困难模式选择 |
| **重置游戏** | 清除 Cookie 存档与所有 localStorage 数据（成就/结局/里程/历程/难度记忆） |
| **成就** | 查看所有成就及解锁状态（隐藏成就未解锁时不显示图标/描述） |
| **历史历程** | 查看最近约 30 次旅途记录（结局、里程、乘客、成就） |
...
| 数值 | 说明 | 初始/范围 |
|------|------|-----------|
| 金币 | 用于商人交易、休息站消费 | 初始 6 |
| 里程 | 本次 / 历史最高 | 0 / localStorage 持久 |
| 后备箱载重 | 物品总重量上限（超载无法继续） | 50 |
| 好感度 | 每位乘客独立 | 0～100，默认 50 |

> **休息站经济**：休息站的「停车休整」（耐久 +10）和「小憩片刻」（舒适 +10）均消耗 **5 金币**；「继续赶路」免费但消耗燃油 -6。
...
| gameOver | `true` 时游戏结束 |
| treasureDrop | `{ id, weight, chance }` 按概率掉落珍品，触发珍品特效（背景光、弹窗、成就） |
...
珍品是稀有的特殊收藏物品，通过特定事件低概率获得或满足条件自动获得，大多拥有独特的被动效果。

| 珍品 | 重量 | 颜色 | 获取方式 | 被动效果 |
|------|------|------|----------|----------|
| 鹿角护符 | 1 | #d4a574 | 鹿好感 ≥90 自动获得 | 每次选择后 20% 概率恢复 5% 舒适 |
| 萤火虫之愿 | 1 | #fbbf24 | 萤火虫之夜事件选择「靠近观赏」 | 夜晚事件时 25% 概率恢复 2~5 耐久 + 1~2 金币 |
| 雨水护符 | 1 | #60a5fa | 暴风雨事件选择「迎着雨冲」 | 雨相关事件免疫所有负面效果 |
| 雾中人的照片 | 1 | #94a3b8 | 浓雾事件选择「开大灯慢行」15% | 雾/沙尘暴等视线受阻事件免疫所有负面效果 |
| 天马星座的流星 | 4 | #c084fc | 夜幕降临选择「仰望星空」12% | 夜晚事件时获得 1 金币（必定触发） |
| 骚福瑞手办 | 1 | #ff69b4 | 车上 ≥2 人且全员好感 ≥70 自动获得 | 每次选择后 25% 概率恢复 8% 燃油 |
| 金马雕像 | 5 | #ffd700 | 古老纪念碑事件 | 每次选择后 25% 概率获得 1~2 金币 |
| 海市蜃楼雕塑 | 5 | #e0a050 | 沙尘暴事件选择「穿越沙尘暴」 | 燃油 ≤10% 时自动恢复 25% 燃油，然后变为「破损的雕塑」 |
| 呓语之书 | 2 | #9f7aea | 古老纪念碑事件 | 可主动使用：下一次事件所有属性效果正负倒转，使用后变为「空白书」 |
| 马头皮卡核心 | -10 | #00ff88 | 马头外星人事件 | 重量 -10（减轻后备箱负担） |
| 钻石稿 | 5 | #b9f2ff | 废弃矿场深处探索 | 无被动，可高价卖出 |
| 繁荣时代的金属碎片 | 8 | #fbbf24 | 废弃村庄/废弃仓库深处 | 持有时商人购买价格降低 25% |
| 沥青滴落装置 | 5 | #4a4a4a | 废弃加油站深处探索 | 每次选择后 2% 概率获得原油 ×1 |
| 猎人徽章 | 2 | #8b7355 | 猎人在车，好感 ≥90，耐久 ≤40 | 可主动使用：完全恢复车辆耐久，使用后变为「黯淡的猎人徽章」 |
| 和平徽章 | 1 | #10b981 | 鹿+猎人同车，好感均 ≥70 | 每次抉择恢复 1% 耐久 + 1% 舒适（必定触发） |

> **变形珍品**：海市蜃楼雕塑 → 破损的雕塑（触发后），呓语之书 → 空白书（使用后），猎人徽章 → 黯淡的猎人徽章（使用后）。变形后失去原有效果，重量变为 0，仅保留纪念价值。

### 5.4 节日物品

| 物品 | 重量 | 效果 |
|------|------|------|
| 马年红包 | 0 | 使用后随机获得 1~6 金币（新游戏自动获得） |

### 5.5 合成配方（CRAFTING_RECIPES）
...
| firefly_blessing | ✨ | 萤火虫祝福 | 获得珍品「萤火虫之愿」 |
| rain_blessing | 🌧️ | 风雨无阻 | 获得珍品「雨水护符」 |
| fog_phantom | 👻 | 雾中来客 | 获得珍品「雾中人的照片」 |
| starfall_pegasus | 🌠 | 星坠天马 | 获得珍品「天马星座的流星」 |
| golden_horse | 🐴 | 点石成金 | 获得珍品「金马雕像」 |
| mirage_sculpture | 🏜️ | 沙海幻影 | 获得珍品「海市蜃楼雕塑」 |
| whisper_book | 📖 | 命运倒转 | 获得珍品「呓语之书」 |
| horsehead_core | 👽 | 第三类接触 | 获得珍品「马头皮卡核心」 |
| diamond_pickaxe | ⛏️ | 矿脉传说 | 获得珍品「钻石稿」 |
| prosperity_fragment | ✨ | 繁荣的回响 | 获得珍品「繁荣时代的金属碎片」 |
| asphalt_drip | 🛢️ | 黑金炼成 | 获得珍品「沥青滴落装置」 |
| saofurry_figure | 💖 | 狂热收藏家 | 获得珍品「骚福瑞手办」 |
...
| ID | 图标 | 成就名 | 条件 |
|----|------|--------|------|
| open_red_packet | 🧧 | 开年大吉 | 打开马年红包 |

### 8.8 结局类（隐藏，11 个）

| ID | 图标 | 成就名 | 对应结局 |
|----|------|--------|----------|
| ending_fuel_empty | ⛽ | 燃油耗尽 | fuel_empty |
| ending_durability_zero | 💥 | 车辆报废 | durability_zero |
| ending_comfort_zero | 😢 | 众叛亲离 | comfort_zero |
| ending_game_over_event | 💀 | 被骚福瑞打爆 | game_over_event |
| ending_perfect_journey | ✨ | 完美旅程 | perfect_journey |
| ending_eternal_friendship | 💜 | 友谊永恒 | eternal_friendship |
| ending_collector | 📦 | 收集者 | collector |
| ending_legendary_driver | 👑 | 传奇司机 | legendary_driver |
| ending_lonely_wanderer | 🌙 | 孤独行者 | lonely_wanderer |
| ending_harmony | 🕊️ | 和谐共存 | harmony |

> 成就总计约 **44 个**，跨档保存在 `localStorage`。
...
新游戏开始前可选择困难模式修正器和奖励。修正器增加难度并消耗点数，奖励提供增益但消耗额外点数。选择后存储至 `sessionStorage`（传递至 game.html）和 `localStorage`（记忆上次选择）。

### 10.2 修正器（HARD_MODE_MODIFIERS）

每个修正器消耗一定分值，首次无存档进入游戏时弹出选择面板。
...
| 🔋 高油耗 | 燃油消耗 +50% | 3 |
| 💔 脆弱 | 所有负面属性效果 ×1.5 | 5 |
| ⚖️ 属性平衡 | 每次选择后三项属性趋向平均值（强制均衡化） | 3 |

### 10.3 奖励（HARD_MODE_BONUSES）

| 标签 | 描述 | 消耗分值 |
|------|------|----------|
| 🍀 幸运加持 | 正面事件概率 +15% | 5 |
| ⚖️ 属性平衡 | 与修正器同名，此处为增益版 | 5 |
| 🎁 珍品猎人 | 珍品掉落概率 +25% | 6 |

### 10.4 选择记忆

- 上次选择的修正器和奖励自动保存至 `localStorage`（`hard_mode_last_tags` / `hard_mode_last_bonuses`）
- 下次打开选择面板时自动恢复上次选择
- 提供「全选 / 全消」快捷按钮
- 重置游戏时清除记忆
...
## 十一、游戏状态（gameState / truckState / inventoryState）
...
- hardModeTags, hardModeBonuses（困难模式标签）
...
- **珍品获得特效**：珍品掉落时显示对应颜色的 backdrop 光效 + 弹窗通知
- **幽灵粒子**：持有「雾中人的照片」时，雾/沙尘暴事件触发半透明幽灵粒子飘散
- **事件主题背景**：部分事件设有视觉主题（如雨滴、雾气、火光等）

### 12.4 UI 交互细节

- **商人/合成/休息界面**：操作后自动保存并恢复滚动位置
- **超载弹窗**：刷新后保留滚动位置；允许超载购买/合成（黄色警告样式），事件结束后统一检查超载
- **状态条显示**：燃油/耐久/舒适的进度条和数字均使用 `Math.max(0, value)` 显示，不显示负值
...
### 13.1 Cookie（`chinese_truck_adventure_save`）
...
| Key | 用途 |
|-----|------|
| chinese_truck_adventure_achievements | 成就解锁记录 |
...
| chinese_truck_adventure_volume | 音频音量 |
| hard_mode_last_tags | 上次困难模式修正器选择 |
| hard_mode_last_bonuses | 上次困难模式奖励选择 |

### 13.3 sessionStorage

| Key | 用途 |
|-----|------|
| hard_mode_tags | 当前游戏的困难模式修正器（JSON） |
| hard_mode_bonuses | 当前游戏的困难模式奖励（JSON） |
